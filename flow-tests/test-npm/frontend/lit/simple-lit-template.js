...
class SimpleLitTemplate extends LitElement {
    ...
    render() {
        ...
        <ul>
         	${this.strings.map(item => html`<li>${item}<button @click="${e => this.$server.deleteString(item)}">Remove</button></li>`)}
         	<li><button id="addString">Add</button></li>
        </ul>
        
        <div>Persons (updated by modifying model list):</div>
        <ul>
         	${repeat(this.persons, person => person.id, (person, index) => html`
         		<li>${index}: ${person.lastName}, ${person.firstName}
         		<button @click="${e => this.$server.deletePerson(person.id)}">Remove</button>
         		</li>
            `)}
